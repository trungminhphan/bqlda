...
class CongVan{
	...
	public function insert(){
		$query = array('id_loaicongvan' => $this->id_loaicongvan ? new MongoId($this->id_loaicongvan) : '',
						...
						'sothutu' => is_numeric($this->sothutu) ? intval($this->sothutu) : $this->sothutu,
						'ngayky' => $this->ngayky,
						'nguoiky' => $this->nguoiky,
						'thoigianxuly' => $this->thoigianxuly,
						'phongxuly' => $this->phongxuly,
	...
	public function get_totalFilter($condition){
		return $this->_collection->find($condition)->sort(array('sothutu'=>1))->count();
	}

	public function get_list_to_position_condition($condition, $position, $limit){
		return $this->_collection->find($condition)->skip($position)->limit($limit)->sort(array('sothutu' => 1));
	}
